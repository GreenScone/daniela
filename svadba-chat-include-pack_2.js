...
var SWMID = {
	...
	send:function(request){
		...
		switch(obj.list){
			...
			break;
			case 2: 
				SWMID.obj_sort_list.pisal = (SWMID.obj_sort_list.pisal==null)?SWMID.sort_list(['age','vip_photo','bleck','contacts'],SWMID.arr_pisal):SWMID.obj_sort_list.pisal;
				...
				if(s!=null&&s[0].updates[0].girl.chats.length>0){
					...
					for(c=0;c<s[0].updates[0].girl.chats.length;c++){
						...
						$.each(chat_act_time,function(i,vfs){
							...
							old_date = old_date.setMinutes(old_date.getMinutes()+4);
...
chrome.extension.onMessage.addListener(function(request, sender, sendResponse) { 
	switch(request.command){
		...
		break;
		case 'get_pisal':
			SWMID.arr_pisal=JSON.parse(localStorage['pisal_list'+$.cookie('user_id')]);
